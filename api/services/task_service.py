...
class TaskService:
    ...
    def get_all_tasks(self):
        if not self._domain.check_access:
    ...
    def move_task(self, task_id, task_data):
        if not self._domain.check_access:
            raise PermissionError("User does not have access to this project.")
        return self._task_management.move_task(task_id, task_data)

    def delete_task(self, task_id):
        if not self._domain.check_access:
            raise PermissionError("User does not have access to this project.")
        return self._task_management.delete_task(task_id)

    def assign_user_to_task(self, task_id, project_member_id):
        if not self._domain.check_access:
            raise PermissionError("User does not have access to this project.")
        return self._task_management.assign_user_to_task(task_id, project_member_id)

    def unassign_user_from_task(self, task_id, project_member_id):
        if not self._domain.check_access:
            raise PermissionError("User does not have access to this project.")
        return self._task_management.unassign_user_from_task(task_id, project_member_id)
